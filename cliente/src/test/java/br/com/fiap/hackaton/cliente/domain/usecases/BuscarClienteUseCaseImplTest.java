...
class BuscarClienteUseCaseImplTest {
    ...
    void setUp() {
        ...
        buscarClienteUseCase = new BuscarClienteUseCaseImpl(
    ...
    class BuscarPorCpf {
        ...
        void deveRetornarCliente_QuandoBuscarPorCpf() {
            ...
            when(buscarClienteRepository.porCpf(cpf)).thenReturn(clienteEsperado);

            Cliente clienteRetornado = buscarClienteUseCase.porCpf(cpf);
            ...
            verify(buscarClienteRepository, times(1)).porCpf(cpf);
        }

        @Test
        void deveRetornarNulo_QuandoClienteNaoEncontradoPorCpf() {
            String cpf = "12345678901";
            when(buscarClienteRepository.porCpf(cpf)).thenReturn(null);
    ...
    class BuscarPorId {
        ...
        void deveRetornarCliente_QuandoBuscarPorId() {
            ...
            Cliente clienteEsperado = ClienteHelper.gerarCliente();
            when(buscarClienteRepository.porId(id)).thenReturn(clienteEsperado);

            Cliente clienteRetornado = buscarClienteUseCase.porId(id);
            ...
            assertThat(clienteRetornado)
                    .extracting(Cliente::getNome)
                    .isEqualTo(clienteEsperado.getNome());
            verify(buscarClienteRepository, times(1)).porId(id);
        }

        @Test
        void deveRetornarNulo_QuandoClienteNaoEncontradoPorId() {
            UUID id = UUID.randomUUID();
            when(buscarClienteRepository.porId(id)).thenReturn(null);
